...
import { FaHome, FaBook, FaList, FaCog, FaMoon, FaSun } from "react-icons/fa";
...
const Sidebar = ({ isOpen, setIsOpen }) => {
  ...
  return (
    <>
      ...
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-gray-900 border-r border-[#CCCCCC] dark:border-gray-800 flex flex-col justify-between transition-transform duration-500
      ...
      `}>
        ...
        <div className="px-4 py-2">
          <button
            onClick={toggleTheme}
            className="w-full flex items-center gap-4 px-4 py-3 rounded-xl transition-all duration-300 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 dark:text-gray-400"
          >
            {isDarkMode ? (
              <>
                <FaSun size={20} className="text-yellow-500" />
                <span className="text-sm font-bold tracking-tight">Light Mode</span>
              </>
            ) : (
              <>
        ...
        {/* ===== BOTTOM (USER) ===== */}
        <div className="p-4 border-t border-[#CCCCCC] dark:border-gray-700">
          <div
            ref={menuRef}
            onClick={() => setShowMenu((p) => !p)}
            className="relative flex items-center gap-3 p-3 rounded-2xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-all duration-300 cursor-pointer group"
          >
            <motion.img
            ...
            <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-gray-800 dark:text-gray-200 truncate group-hover:text-blue-600 transition-colors">
                    {user?.name || "Guest"}
                </p>
            ...
            <AnimatePresence>
              {showMenu && (
                <motion.div
                  ...
                  className="absolute bottom-full left-0 w-full mb-3 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl shadow-2xl py-2 z-50 overflow-hidden"
                >
                  <button
                    onClick={() => { navigate("settings"); }}
                    className="w-full text-left px-4 py-3 text-sm text-gray-700 dark:text-gray-300 hover:bg-blue-50 dark:hover:bg-gray-800 font-bold flex items-center gap-3 transition-colors"
                  >
                    <FaCog /> Settings
                  </button>
                  <button
                    ...
                    className="w-full text-left px-4 py-3 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 font-bold flex items-center gap-3 transition-colors border-t border-gray-50 dark:border-gray-800"
                  >
                    <FiMic /> Sign Out
                  </button>
                </motion.div>
